...
- 📂 **绑定项目目录**（v1.1）—— 建群时可选填一个项目目录，群成员直接在你的项目里工作（读代码、跑测试、改文件）；留空则用 Hub 独立目录。
- 🔌 **自定义成员**（v1.1）—— 任何能在 PowerShell 里启动的交互式 AI CLI（公司内部网关、ollama、ssh 工具…）都能保存为群聊成员：设置里填「名称 + 启动命令」，建群下拉即可选。
