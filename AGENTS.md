...
| AI 干到一半停下等确认 | 默认是安全模式（敏感操作会请求审批）→ 「⚙️ 设置 → 执行权限」可切「完全自动」（跳过审批，风险自担） |

---

## 进阶 · 替用户添加「自定义 AI 成员」（agent 可代配）

用户如果有 Hub 未原生集成的 AI（公司内部网关 CLI、ollama、任何交互式命令行工具），你可以替 TA 配置成群聊成员。两种方式：

**方式 1（推荐）**：让用户打开「⚙️ 设置 → 自定义成员」，填「名称 + 启动命令」点添加。

**方式 2（agent 直接改配置文件）**：编辑 `~/.claude-session-hub/config.json`，添加 `custom_members` 数组（Hub 重启后生效；若 Hub 正在运行建议让用户重启）：

```json
{
  "custom_members": [
    { "id": "myai", "name": "内部助手", "command": "my-company-ai --chat" }
  ]
}
```

字段约束：`id` 为 1-32 位字母数字/`-`/`_`（唯一）；`name` ≤40 字；`command` ≤500 字，必须是**能在 PowerShell 里启动并进入交互模式**的命令。

行为须知（告知用户）：
- 群聊消息会被**压成单行**发给该 CLI（任意 CLI 对多行输入的约定不统一）；
- 回答按「输出静默 8 秒」判定完成——适合一问一答式 CLI；流式长输出也兼容（停止输出 8 秒后收卡）；
- 该成员无法预检可用性（命令是否有效由用户保证），起不来时卡片会一直等待，可在 PTY 视图排查。
